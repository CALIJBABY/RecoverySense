...
def generate_demo_data(config: dict[str, Any], participants: int = 12, minutes: int = 12) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Generate deterministic, non-clinical data for testing the pipeline only."""
    ...
    for participant_number in range(participants):
        ...
        timestamps = base_time + pd.to_timedelta(time_seconds + participant_number * 86400, unit="s")
        ...
        heart_rate = 70 + rng.normal(0, 2.5, samples) + 3 * np.sin(2 * np.pi * time_seconds / 60)
        activity_scale = 0.025 + 0.015 * np.sin(2 * np.pi * time_seconds / 30) ** 2
        ...
        if has_positive_event:
            ...
            accel_y[event_mask] += 0.16 * np.cos(2 * np.pi * 1.4 * time_seconds[event_mask])
        ...
        ema_rows.append(
            {
                ...
                "craving_score": int(rng.integers(8, 11) if has_positive_event else rng.integers(0, 5)),
